...
describe('Users API', () => {
    ...
    test('GET /users returns paginated results for admin', async () => {
        ...
        const res = await request(app).get('/users').set('x-auth-token', token);
        expect(res.statusCode).toBe(200);
    ...
    test('POST /auth/create creates a new user', async () => {
        const newUser = { username: 'jane', password: 'password', role: 'user', fullName: 'Jane Doe' };
        const res = await request(app).post('/auth/create').set('x-auth-token', token).send(newUser);
    ...
    test('PUT /users/:id updates user and DELETE removes user', async () => {
        ...
        const delRes = await request(app).delete(`/users/${user._id}`).set('x-auth-token', token);
    ...
    test('Manager can create employee but not manager/admin', async () => {
        ...
        const createManagerRes = await request(app).post('/auth/create').set('x-auth-token', token).send(manager);
        expect(createManagerRes.statusCode).toBe(201);

        // login as manager
        const loginRes = await request(app).post('/auth/login').send({ username: 'manager1', password: 'pass1234' });
        ...
        const res1 = await request(app).post('/auth/create').set('x-auth-token', managerToken).send(emp);
        expect(res1.statusCode).toBe(201);

        // manager cannot create manager or admin
        const res2 = await request(app).post('/auth/create').set('x-auth-token', managerToken).send({ username: 'manager2', password: 'pass1234', role: 'manager' });
        expect(res2.statusCode).toBe(403);

        const res3 = await request(app).post('/auth/create').set('x-auth-token', managerToken).send({ username: 'admin2', password: 'pass1234', role: 'admin' });
    ...
    test('HR can create employee but not manager', async () => {
        ...
        const createHRRes = await request(app).post('/auth/create').set('x-auth-token', token).send(hr);
        expect(createHRRes.statusCode).toBe(201);

        // login as HR
        const loginRes = await request(app).post('/auth/login').send({ username: 'hr1', password: 'pass1234' });
        const hrToken = loginRes.body.token;

        // hr can create employee
        const emp = { username: 'employee2', password: 'pass1234', role: 'employee' };
        const res1 = await request(app).post('/auth/create').set('x-auth-token', hrToken).send(emp);
        expect(res1.statusCode).toBe(201);

        // hr cannot create manager
        const res2 = await request(app).post('/auth/create').set('x-auth-token', hrToken).send({ username: 'manager3', password: 'pass1234', role: 'manager' });
        expect(res2.statusCode).toBe(403);
    });

    test('Normal user cannot create user accounts', async () => {
        // Create a normal user via admin
        await request(app).post('/auth/create').set('x-auth-token', token).send({ username: 'joe', password: 'userpass', role: 'user' });
        const loginRes = await request(app).post('/auth/login').send({ username: 'joe', password: 'userpass' });
        const userToken = loginRes.body.token;

        const res = await request(app).post('/auth/create').set('x-auth-token', userToken).send({ username: 'userX', password: 'userpassX', role: 'user' });
    ...
    test('Manager cannot change role of a user', async () => {
        ...
        await request(app).post('/auth/create').set('x-auth-token', token).send(manager);
        const loginRes = await request(app).post('/auth/login').send({ username: 'mgr2', password: 'pass1234' });
        const managerToken = loginRes.body.token;

        const employee = { username: 'emp2', password: 'pass1234', role: 'employee' };
        const createRes = await request(app).post('/auth/create').set('x-auth-token', token).send(employee);
        expect(createRes.statusCode).toBe(201);

        const emp = await User.findOne({ username: 'emp2' });

        // manager tries to change role to manager
        const res = await request(app).put(`/users/${emp._id}`).set('x-auth-token', managerToken).send({ role: 'manager' });
        expect(res.statusCode).toBe(403);
    });
    ...
    test('Admin cannot delete self', async () => {
        ...
        const res = await request(app).delete(`/users/${adminUser._id}`).set('x-auth-token', token);
        expect(res.statusCode).toBe(400);
    });

    test('Register endpoint is admin-only', async () => {
        // Unauthenticated should receive 401
        const resUnauth = await request(app).post('/auth/register').send({ username: 'anon', password: 'x' });
        ...
        const resAdmin = await request(app).post('/auth/register').set('x-auth-token', token).send({ username: 't2', password: 'pass123' });
